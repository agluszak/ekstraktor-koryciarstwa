...
def test_wp_lubczyk(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    target_assert(
        subtests, any("Hołownia" in e for e in entities), "Should recover Szymon Hołownia"
    )
...
def test_pleszew24_stadnina(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    """
    key = (
        "pleszew24.info__pl__12_biznes__16076_radna-powiatowa-z-posada-zmiana-prezesa-"
        "slynnej-panstwowej-stadniny-koni"
    )
    if key not in benchmark_results:
        pytest.skip(f"{key} not found")
...
def test_natura_tour(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    """
    key = (
        "wiadomosci.onet.pl__kraj__tak-psl-obsadzil-panstwowa-spolke-prace-dostal-"
        "min-29-letni-brat-wiceministra__ezt8y9t"
    )
    if key not in benchmark_results:
        pytest.skip(f"{key} not found")
...
def test_nfosigw_odpartyjnienie(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    """
    key = (
        "wiadomosci.wp.pl__odpartyjnienie-rad-nadzorczych-nie-tak-mialo-byc-wyglada-"
        "to-bardzo-zle__6996280410176160a"
    )
    if key not in benchmark_results:
        pytest.skip(f"{key} not found")
...
def test_sloma_olsztyn(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    """
    key = (
        "olsztyn.tvp.pl__41863255__z-wiceprezydenta-na-wiceprezesa-jaroslaw-sloma-"
        "w-zarzadzie-olsztynskich-wodociagow"
    )
    if key not in benchmark_results:
        pytest.skip(f"{key} not found")
...
def test_tvn24_siemianowice(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    """
    key = (
        "tvn24.pl__polska__kolesiostwo-i-rozdawanie-posad-miasto-umiera-radna-po-ze-"
        "slaska-pisze-do-premiera-ra323735-ls3431831__webarchive_20250427191848"
    )
    if key not in benchmark_results:
        pytest.skip(f"{key} not found")
...
def test_dziennik_zachodni_bytom(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    """
    key = (
        "dziennikzachodni.pl__nepotyzm-w-bytomiu-radni-reprezentujacy-pis-"
        "zawiadomienie-cba__c1-16375383"
    )
    if key not in benchmark_results:
        pytest.skip(f"{key} not found")
...
def test_wfosigw_lublin_xfail(benchmark_results: dict[str, Any], subtests: Subtests) -> None:
    ...
    """
    key = (
        "wiadomosci.onet.pl__lublin__nowe-wladze-wfosigw-w-lublinie-bez-konkursu-i-"
        "bez-wysluchania-kandydatow__cpw9ltt"
    )
    if key not in benchmark_results:
        pytest.skip(f"{key} not found")
