...
def test_party_cannot_become_appointment_destination() -> None:
    ...
    assert not any(fact.fact_type == "APPOINTMENT" for fact in extracted.facts)
    assert not any(relation.relation_type == "APPOINTED_TO" for relation in extracted.relations)
...
def test_initials_and_paragraph_carryover_support_governance_fact() -> None:
    ...
    appointments = [fact for fact in extracted.facts if fact.fact_type == "APPOINTMENT"]
