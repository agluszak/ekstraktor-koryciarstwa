...
for art in articles:
    ...
    for fact in v1_data.get('facts', []):
        subj = v1_entities.get(fact.get('subject_entity_id'), {}).get('canonical_name', fact.get('subject_entity_id', ''))
        obj = v1_entities.get(fact.get('object_entity_id'), {}).get('canonical_name', fact.get('object_entity_id', ''))
        role_val = fact.get('role') or fact.get('value_text', '')
        if role_val is None: role_val = ""
    ...
    for assessment in v2_data.get('fact_assessments', []):
        if assessment['assessment']['score'] >= 0.5:
            ...
            for arg in cand.get('arguments', []):
                role = arg.get('role')
                if 'entity_id' in arg:
                    val = v2_entities.get(arg['entity_id'], {}).get('canonical_hint', arg['entity_id'])
                else:
                    val = arg.get('value', '')
            ...
            ev_text = str(evidence_ids) # placeholder, since we don't load the full sentences mapping here simply
            
            out.write(f"| {cand.get('kind')} | {assessment['assessment']['score']:.2f} | {', '.join(args_str)} | {ev_text} |\n")
    ...
    if "ai42" in art['filename']:
        out.write("- **What V1 has that V2 misses**: V1 extracts the appointment of Rafał Dobosz (Pomoc Administracyjna) and his kinship tie to Wójt Artur Sosna. V2 missed these entirely (no facts scored >= 0.5).\n")
        out.write("- **What V2 has that V1 misses**: None. V2 correctly identified relevance but produced no high-scoring facts.\n")
    elif "charsznicy" in art['filename']:
        out.write("- **What V1 has that V2 misses**: V1 extracts the kinship ties ('partnerka wójta', 'przyszły teść') and their appointments to Urząd Gminy and USC. V2 missed these ties.\n")
        out.write("- **What V2 has that V1 misses**: V2 extracts appointments to Gminny Ośrodek Kultury.\n")
        out.write("- **False Positives**: V2 incorrectly resolved the appointee to 'Tomasz Kościelniak' (the Wójt) instead of the actual director (likely Szymon Kubit, who V1 identified as party member/election candidate).\n")
    elif "nepotyzm-w-bytomiu" in art['filename']:
        out.write("- **What V1 has that V2 misses**: V1 captures multiple political office roles (Radny).\n")
        out.write("- **What V2 has that V1 misses**: V2 successfully extracts the exact contract amount (397 496,95 zł) for Wnuk Consulting & PEC. V2 also captures the CBA referral with high precision.\n")
        out.write("- **False Positives**: V2 extracted absurd governance appointments: Maciej Bartków to 'Facebook' and 'Państwowej Komisji Wyborczej' and 'Super'. This is because it misinterpreted actions like posting on Facebook or reporting to PKW as taking governance roles.\n")
    elif "radomszczanska" in art['filename']:
        out.write("- **What V1 has that V2 misses**: V1 identifies the compensation as coming from AMW Rewita.\n")
        out.write("- **What V2 has that V1 misses**: V2 correctly maps the kinship tie between exactly resolved names: 'Mirella Zugaj' and 'Radka Zugaja' (spouse), whereas V1 had noisy nominals like 'Żona Radka Zugaja'. V2 also correctly extracts the party affiliation and governance appointment (Rada Nadzorcza).\n")
        out.write("- **False Positives**: V2 attributes the funder of the compensation to 'PO' (Platforma Obywatelska) instead of the company AMW Rewita.\n")
        out.write("- **Notable Improvements**: V2 successfully linked the nominal kinship ('żona') to the resolved entity 'Mirella Zugaj', proving the reference resolution stages work well here.\n")
