...
* `exclude_fact_kinds` *(list[str] | None, default: None)*: A blacklist of main fact categories (e.g., `["public_contract", "funding"]`). Facts matching these kinds will not appear in the final output.
* `exclude_relationships` *(list[str] | None, default: None)*: A blacklist of specific relationship details (e.g., `["friend", "associate"]`). Any tie fact containing these specific relationship subtypes will be excluded.
