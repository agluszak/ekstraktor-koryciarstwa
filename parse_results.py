...
for name, (v1_path, v2_path) in articles.items():
    ...
    for fact in v1_data.get('facts', []):
        ...
        out.write(f"- `{fact.get('fact_type')}` | Subj: {subj} | Obj: {obj} | Role: {role} | Val: {val} | Ev: {ev}\n")
    ...
    for assessment in v2_data.get('fact_assessments', []):
        if assessment['assessment']['score'] >= 0.5:
            ...
            out.write(f"- `{cand.get('kind')}` (score: {assessment['assessment']['score']:.2f}) | {', '.join(args_str)} | Ev_ids: {evidence_ids}\n")
    out.write("\n---\n")
